...
export function Dashboard() {
  ...
  const loadStats = async () => {
    try {
      // 并行加载多个数据源，与 public/index.html 保持一致
      const [keysRes, tokensRes, keyStatsRes, tokenStatsRes, statusRes] = await Promise.all([
        adminApi.getKeys(),
        adminApi.getTokens(),
        adminApi.getKeyStats().catch(() => ({ success: true, data: { totalRequests: 0 } })),
        adminApi.getTokenStats().catch(() => ({ success: true, data: { enabled: 0, disabled: 0 } })),
        adminApi.getStatus().catch(() => null),
      ])

      // 密钥数据
      if (keysRes.success && keysRes.data) {
        const keys = keysRes.data as unknown[]
        setStats(prev => ({
          ...prev,
      ...
      if (tokensRes.success && tokensRes.data) {
        const tokens = tokensRes.data as unknown[]
        setStats(prev => ({
          ...prev,
      ...
      if (statusRes && statusRes.success && statusRes.data) {
        ...
        setStats(prev => ({
          ...
          idleStatus: (data.idle as string) === '空闲模式' ? '空闲' : '活跃',
